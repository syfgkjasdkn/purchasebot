...
5. Set `WEBHOOK` env var to the webhook url which the bot will use to register itself with telegram at startup. Note that the bot [will append the telegram token to the end of this url](https://core.telegram.org/bots/faq#how-can-i-make-sure-that-webhook-requests-are-coming-from-telegr) to avoid random requests being processed as those from telegram.
...
docker run purchasebot \
  ...
  -e WEBHOOK=https://13.13.13.13:51762/tgbot \
